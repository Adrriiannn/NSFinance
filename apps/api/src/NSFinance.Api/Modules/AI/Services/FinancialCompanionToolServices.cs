...
public sealed class UserFinancialContextProfileService(
    ...
    ISpendingAnalysisService spendingAnalysisService) : IUserFinancialContextProfileService
{
    public async Task<UserFinancialContextSnapshot> GetOrCreateAsync(Guid userId, CancellationToken cancellationToken)
    {
        var existing = await dbContext.UserFinancialContextProfiles
            .AsNoTracking()
        ...
        if (existing is not null)
        {
            return new UserFinancialContextSnapshot(
                existing.Country,
                existing.Currency,
                existing.MonthlyIncomeRange,
                existing.KnownObligationsJson,
                existing.BudgetStructureJson,
                existing.ActivePlansJson,
                existing.SpendingTendenciesJson,
                existing.CategoryFlexibilityMarkersJson,
                existing.AdviceStylePreference);
        }
        ...
        var spending = await spendingAnalysisService.AnalyzeAsync(userId, 60, cancellationToken);

        var profile = new UserFinancialContextProfile
        {
            UserId = userId,
            Country = string.IsNullOrWhiteSpace(user.CountryRegion) ? "ZZ" : user.CountryRegion.Trim().ToUpperInvariant(),
            Currency = string.IsNullOrWhiteSpace(user.PreferredCurrency) ? summary.Currency : user.PreferredCurrency.Trim().ToUpperInvariant(),
            MonthlyIncomeRange = DeriveIncomeRange(summary.IncomeLast30Days),
            KnownObligationsJson = JsonSerializer.Serialize(recurring.Items),
            BudgetStructureJson = JsonSerializer.Serialize(new
            {
            ...
            ActivePlansJson = JsonSerializer.Serialize(await dbContext.ExpensePlans
                .AsNoTracking()
                ...
                .ToListAsync(cancellationToken)),
            SpendingTendenciesJson = JsonSerializer.Serialize(new
            {
            ...
            AdviceStylePreference = NormalizeAdviceStyle(user.Preferences?.AdviceTonePreference),
