...
export default function ExpensePlanPublicationDetailScreen() {
  ...
  const { getPublicationById, togglePublicationLike, usePublication, unpublishPublication, rescanPublication } = useExpensePlanning();

  const publication = getPublicationById(publicationId);

  if (!publication) {
    return (
      <ExpenseTrackerMiniAppScreen title="Community plan">
    ...
    <ExpenseTrackerMiniAppScreen title="Community plan">
      ...
      <GlassCard style={styles.sectionCard}>
        ...
        <View style={styles.actionStack}>
          ...
          <PrimaryButton label="Use this plan" onPress={() => {
            const imported = usePublication(publication.id);
            if (!imported) {
              Alert.alert("Could not use plan", "This plan is not available right now.");
              return;
            }
            router.push(`/(tabs)/planner/expense-tracker/plan/${imported.id}` as never);
          }} />
          <PrimaryButton label="Report plan" onPress={() => router.push(`/(tabs)/planner/expense-tracker/community/report/${publication.id}` as never)} />
        </View>
      </GlassCard>
      ...
      {isCreator ? (
        <GlassCard style={styles.sectionCard}>
          ...
          <View style={styles.actionStack}>
            <PrimaryButton label="Edit public metadata" onPress={() => router.push({ pathname: "/(tabs)/planner/expense-tracker/community/publish", params: { publicationId: publication.id } } as never)} />
            ...
            <PrimaryButton label="Unpublish" onPress={() => {
              ...
              router.replace("/(tabs)/planner/expense-tracker/community/dashboard" as never);
            }} />
          </View>
        </GlassCard>
      ) : null}
